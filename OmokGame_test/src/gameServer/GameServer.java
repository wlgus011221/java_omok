...
public class GameServer extends Thread {
	...
	private void analysisChatData(ChatData data) {
		...
		case ChatData.ENTER:
			...
			if (!version.equals(""+MainServer.VERSION)) {
				...
				// 3번째부터 들어오는 유저를 관전자로 설정
		        if (server.getUserList().getCollection().size() >= 3) {
		            GameServer thirdUser = server.getUserList().getCollection().get(2);
		            thirdUser.setSpectator(true);
		        }
			}
	...
	private void analysisLobbyData(LobbyData data) {
		...
		case LobbyData.ENTER_TO_ROOM:
			...
			if (userCount < 4) { // 이걸로 2명만 들어가던 방을 최대 4명으로 바꿈
